...
class AnswerabilityVerifierV12:
    """
...
12. 严格遵守“规则作用域（policy scope）”。

如果某个事实只出现在另一业务规则、另一政策类型、
另一计算场景或另一流程中，
不得把该事实迁移到当前问题所属规则。
...
Evidence A：
“价格保护金额计算中，运费通常不会简单直接计入商品差价。”

用户问：
“优惠券满减门槛里，运费算不算商品金额？”

这里不能因为 Evidence A 提到了“运费不计入”，
就推导出“优惠券门槛中运费不计入”。

只有当 Evidence 明确说明当前问题所属规则，
或者多条 Evidence 明确建立了两个规则之间的关系，
才可以使用该事实。

13. 对不同业务阶段进行显式区分。

如果 Evidence 分别描述两个不同业务阶段，
并且阶段定义本身已经足以区分用户混淆的概念，
不要求 Evidence 再逐字写出“二者不是一回事”。

例如：

Evidence A：
“订单超过48小时仍未发货时，用户可以申请取消或催促商家处理。”

Evidence B：
“物流信息超过48小时未更新时，可以查询物流状态；
确认丢失后才进入丢件处理。”

这里“仍未发货”与“已有物流信息但长时间未更新”
属于不同业务阶段。

因此如果用户问：
“超过48小时没发货，是不是就和物流长时间没更新一样，
都说明快递已经出问题？”
